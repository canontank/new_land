...
// 아이콘 버튼 토글을 통해 실거래가 뷰와 접속 로그 뷰를 전환합니다.
document.addEventListener('DOMContentLoaded', () => {
  const toggleBtn = document.getElementById('toggle-view-btn');
  const viewRealestate = document.getElementById('view-realestate');
  const viewLog = document.getElementById('view-log');
  const iconLog = document.getElementById('icon-log');
  const iconRealestate = document.getElementById('icon-realestate');
  ...
  if (toggleBtn) {
    ...
    toggleBtn.addEventListener('click', () => {
      ...
      if (isLogView) {
        // 접속 로그 뷰로 전환
        viewRealestate.style.display = 'none';
        viewLog.style.display = 'block';
        iconLog.style.display = 'none';
        iconRealestate.style.display = 'block';
        toggleBtn.title = '실거래가 조회';
        ...
        // 실거래가 뷰로 전환
        viewRealestate.style.display = 'block';
        viewLog.style.display = 'none';
        iconLog.style.display = 'block';
        iconRealestate.style.display = 'none';
        toggleBtn.title = '접속 로그 조회';
      }
    });
  } else {
    // 버튼이 없는 페이지(혹시 모를)에서는 바로 로그를 가져옴
    fetchLogs();
  }
});

async function fetchLogs() {
  const spinner = document.getElementById('log-loading-spinner') || document.getElementById('loading-spinner');
  const logContainer = document.getElementById('log-container');

  if (!spinner || !logContainer) return;
